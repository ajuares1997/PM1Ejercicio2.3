    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(c.getCount()>0)
        {
            ...
            while(c.isAfterLast()==false)
            {
                ...
                @SuppressLint("Range") byte[] bytes=c.getBlob(c.getColumnIndex(Tabla.blobImagen));
                @SuppressLint("Range") String descripcion=c.getString(c.getColumnIndex(Tabla.descripcion));
